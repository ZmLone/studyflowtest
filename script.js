...
window.renderLeaderboardList = function() {
    ...
    if (myIndex > -1) {
        allPlayers[myIndex] = { ...allPlayers[myIndex], ...myStats, displayName: myName };
    } else {
        allPlayers.push({ id: myId, displayName: myName, ...myStats, currentExam: state.nextExam.name });
    }
    ...
    if(sortedData.length === 0) {
        container.innerHTML = `<div class="p-8 text-center text-slate-400">Season Starting...</div>`;
        return;
    }
    ...
    if(sortedData.length > 0) {
        ...
        html += `<div class="flex justify-center items-end gap-2 md:gap-4 mb-10 mt-4 px-2">`;
    ...
    if(sortedData.length > 3) {
        ...
        sortedData.slice(3).forEach((user, index) => {
            ...
            <div class="flex items-center justify-between p-3 rounded-xl border ${isMe ? 'bg-brand-50 dark:bg-brand-900/20 border-brand-200 dark:border-brand-800' : 'bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800'} shadow-sm">
                ...
                <div class="text-right">
                    <div class="text-sm font-bold text-slate-900 dark:text-white">${user.overallScore || 0}</div>
